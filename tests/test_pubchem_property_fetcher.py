...
def test_fetch_cid_bulk() -> None:
    assert fetch_cid_bulk(["CCC", "CCO", "CC(C)Cc1ccc(C(C)C(=O)O)cc1"], requests.Session()) == {
        "CCC": 6334,
        "CCO": 702,
...
def test_fetch_properties_bulk() -> None:
    assert fetch_properties_bulk(
        [6334, 702, 3672], ["IUPACName", "MolecularWeight"], requests.Session()
    ) == {
        6334: {"IUPACName": "propane", "MolecularWeight": "44.10"},
        702: {"IUPACName": "ethanol", "MolecularWeight": "46.07"},
        3672: {"IUPACName": "2-[4-(2-methylpropyl)phenyl]propanoic acid", "MolecularWeight": "206.28"},
    }


def test_fetch_synonyms_bulk() -> None:
    cid_to_syns = fetch_synonyms_bulk([6334, 702, 3672], requests.Session())
    ...
    assert cid_to_syns == {
        ...
        3672: ["ibuprofen", "IBUPROFEN, (+-)-", "Motrin"],
    }
...
def test_main(tmp_path: Path) -> None:
    ...
    assert pl.read_csv(tmp_path / "output.csv").equals(
